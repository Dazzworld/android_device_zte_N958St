...
LINEEND=" \\"
COUNT=`wc -l ../N958St/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../N958St/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../N958St/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
